...
def adabypass(X, Y, omega, omega_min, theta_init, C):
    ...
    omega_ser = pd.Series(omega)
    omega_arr = ago.np.array(omega_ser)    
    omega_ts = ago.np.empty((T, 3))
    ...
    for t in range(T):
        ...
        if t > 0:
            omega_ts[t] = omega_ts[t-1] +  C * theta_old[1] * stdesc_dir
        else:
            omega_ts[t] = omega_arr +  C * theta_old[1] * stdesc_dir
    ...
    return {'pred_mean': m, 'pred_var': ago.np.array(V), 'pred_err': e_hat, 'weights': w, 'theta': theta, 'omega': omega_ts}